...
【與 TC Toolbox 的關係】（2026-07-30）
本腳本刻意維持「純 SND 原生實作」，不改走 TC Toolbox 的園圃 IPC，原因：
  * 本腳本同時支援庭院「園圃」與各種「花盆／花圃」（靠顯示名稱比對）；
    TC Toolbox 的園圃模組只認庭院園圃地壟（EObj DataId 2003757），
    改過去會直接失去花盆支援。
  * 本腳本已實機驗證、且已針對台服訊息措辭調校過，沒有理由承擔改寫的回歸風險。
若想改用 TC Toolbox 的流程（省掉自己戳選單、UI 變動由它吸收），請用整合版範本
_logs/snd-gardening-tc-template.lua：它會自動偵測 TC Toolbox，沒裝就退回原生路徑。
]]
